...
.PHONY: dev
dev:			## Lunch all services with dev profile
	$(call ensure_user_uid)
ifeq (${NVIDIA_GPU_AVAILABLE}, true)
	$(call COMPOSE_GPU,$(call GET_COMPOSE_PARAMS) COMPOSE_PROFILES=${PROFILE},${COMPOSE_FILES} -f docker-compose-dev.yml)
else
	$(call COMPOSE_CPU,$(call GET_COMPOSE_PARAMS) COMPOSE_PROFILES=${PROFILE},${COMPOSE_FILES} -f docker-compose-dev.yml)
endif
...
.PHONY: build-test
build-test:				## Build image for integration test
	$(call BUILD_TEST)
