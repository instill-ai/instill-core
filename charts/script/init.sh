...
kubectl wait --for=condition=ready pod $(kubectl get pods -n pg-sql -o 'jsonpath={.items..metadata.name}') -n pg-sql --timeout=300s
...
kubectl wait --for=condition=ready pod $(kubectl get pods -n temporal -o 'jsonpath={.items..metadata.name}') -n temporal --timeout=300s
